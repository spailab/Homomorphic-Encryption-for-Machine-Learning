...
int main() {
    ...
    FILE* secret_key = fopen("secretLR.key","wb");
    export_tfheGateBootstrappingSecretKeySet_toFile(secret_key, key);
    fclose(secret_key);

    //export the cloud key to a file (for the cloud)
    FILE* cloud_key = fopen("cloudLR.key","wb");
    ...
    double p[5] = {0.75, 0.43, 0.22, 0.15, 0.69};
    ...
    printf("Hi there! Today, I will ask the cloud what is the Logistic regeression of :\n ");
    ...
    FILE* cloud_data = fopen("cloudLR.data","wb");
    for (int i = 0; i < N; i++){
	    for (int j = 0; j < length; j++){
