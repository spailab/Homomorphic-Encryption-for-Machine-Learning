...
int main() {
    ...
    FILE* cloud_key = fopen("cloudNE.key","rb");
    ...
    FILE* cloud_data = fopen("cloudNE.data","rb");
    //read ciphertext1
    for (int i = 0; i < N; i++)
    {
    ...
    HomLinRegNE(result, ciphertext1,ciphertext2, N, length, bk); 
    ...
    FILE* answer_data = fopen("answerNE.data","wb");
    for (int i = 0; i < res_param; i++)
    {
