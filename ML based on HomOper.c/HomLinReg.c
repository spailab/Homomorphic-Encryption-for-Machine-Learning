# Linear Regression
void HomLinReg(LweSample* res[2], LweSample **x, LweSample **y, const int N, const int length, const TFheGateBootstrappingCloudKeySet* bk){
	...
	for(int k=0;k<iter;k++)
	{
		...
		for(int i=0;i<N;i++)
		{
			...
			HomMulti(L4[i], L3[i], x[i], length, bk);	// {[(beta0 + beta1 * x1) - y1]} * x1
			HomAdd(S1, S1, L3[i], length, bk);	// sum [{[(beta0 + beta1 * x1) - y1]} * x1} for beta0
